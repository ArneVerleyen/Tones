...
const Chords = () => {
    ...
    }, [settings.augmented, settings.diminished, settings.dominant_seventh, settings.major, settings.major_seventh, settings.minor, settings.minor_seventh, settings.sus2, settings.sus4]);
    ...
    const handlePlay = useCallback(() => {
        ...
        if (practiseChords[musicalInterval - 1] === 'minor') {
            ...
            fourth = false;
        } else if (practiseChords[musicalInterval - 1] === 'minor_seventh') {
            console.log('minor7');
            second = allIntervals[tone - 1][3] + intervalOctave.toString();
            if (allIntervals[tone - 1][0] === 'B'|| allIntervals[tone - 1][0] === 'Bb' || allIntervals[tone - 1][0] === 'A') {
                second = allIntervals[tone - 1][3] + (intervalOctave + 1).toString();
